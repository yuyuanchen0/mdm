...
def recursive_split(tokens, max_length, delimiter_tokens):
    ...
    if not split_candidates:
        ...
        return [tokens[i:min(i + max_length, len(tokens))] for i in range(0, len(tokens), max_length)]
...
def preprocess_batch(batch, pad_token, max_length, delimiter, detokeniser, tokeniser):
    ...
    for text in batch["text"]:
        ...
        all_input_ids.extend([c + [pad_token] * (max_length - len(c)) if len(c) < max_length else c for c in chunks])
        all_lengths.extend([len(chunk) for chunk in chunks])

    print(len(all_input_ids), len(all_lengths))
    return {
...
def setup_tokeniser_from_dataset(dataset_name: str):
    tokeniser = None
    match dataset_name:
        case "wikitext2" | "openwebtext" | "EleutherAI/proof-pile-2":
            # tokeniser = setup_tokeniser("gpt2")
            tokeniser = AutoTokenizer.from_pretrained("GSAI-ML/LLaDA-8B-Base", use_fast=True)
        case "dclm":
...
def get_text_dataset(
    ...
    match name:
        case "EleutherAI/proof-pile-2":
            ds_all = load_dataset(name, "default", cache_dir=cache_dir, trust_remote_code=True)
            train_ds = ds_all["train"]
            if split in ["train", "validation"]:
                split_data = train_ds.train_test_split(test_size=0.02, seed=42)
                dataset = split_data["train"] if split == "train" else split_data["test"]
            else:
        ...
        case "openwebtext":
            ds_all = load_dataset(name, cache_dir=cache_dir, trust_remote_code=True)
            train_ds = ds_all["train"]
            if split in ["train", "validation"]:
                split_data = train_ds.train_test_split(test_size=0.02, seed=42)
                dataset = split_data["train"] if split == "train" else split_data["test"]
    ...
    if filter_max_length:
        ...
        tokenised_dataset = dataset.map(
            ...
            batched=True,
            num_proc=num_proc,
            remove_columns=["text", *drop_columns],
        )
        tokenised_dataset = tokenised_dataset.with_format("torch")
...
if __name__ == "__main__":
    ...
    proof_ds = get_text_dataset("EleutherAI/proof-pile-2", "train", drop_columns=["meta"])
    owt_ds = get_text_dataset("openwebtext", "train")
    
    ds = datasets.concatenate_datasets([owt_ds, proof_ds])
    ds.save_to_disk(f"/n/netscratch/albergo_lab/Everyone/jay_brian/datamix")
    print("✔ saved tokenised dataset to disk")
